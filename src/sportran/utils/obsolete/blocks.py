...
try:
    ...
    log.write_log(
        "Error: cannot import inset_axes (will not be able to plot some parts of the plots)"
    )
...
def main():
    ...
    for fname in sys.argv[ff:]:
        if os.path.isfile(fname + ".psd.npy"):
            ...
            periodograms.append(
                np.loadtxt(fname + ".psd.dat", usecols=(3, 4), unpack=True)
            )
        if periodograms[-1].shape != periodograms[0].shape:
            log.write_log(
                fname,
                " not used (inconsistent shape with firts element)",
                periodograms[-1].shape,
        ...
        if os.path.isfile(fname + ".cepstral.npy"):
            ...
            cepstrals.append(
                np.loadtxt(fname + ".cepstral.dat", usecols=(4, 5), unpack=True)
            )
    ...
    logfile.write("Mean value of kappa_scale: {}\n".format(np.mean(kappa_scales)))
    logfile.write("Mean value of minimum of AIC: {}\n".format(aic_KminM))
    logfile.write("\n")
    logfile.write(
        "Mean value and standard deviation of kappa(aic_Kmin):                    {} +/- {}\n".format(
            np.mean(kappas_Kmin), np.std(kappas_Kmin)
        )
    )
    logfile.write(
        "Mean value of calculated statistical error per block of kappa(aic_Kmin): {}\n".format(
            np.mean(kappas_Kmin_std)
        )
    )
    logfile.write("\n")
    logfile.write(
        "Mean value and standard deviation of L0(aic_Kmin):                       {} +/- {}\n".format(
            np.mean(l0s), np.std(l0s)
        )
    )
    logfile.write(
        "Mean value of calculated statistical error per block of L0(aic_Kmin):    {}\n".format(
            np.mean(l0s_std)
        )
    )
    logfile.write("\n\n===========")
    logfile.write("Values at mean value of aic_Kmin:\n")
    logfile.write("\n")
    logfile.write(
        "Mean value and standard deviation of kappa(aic_Kminm):                   {} +/- {}\n".format(
    ...
    np.savetxt(
        output + ".mean_cepstral",
        np.c_[mean_cepstral[0], std_cepstral[0], mean_cepstral[1], std_cepstral[1]],
    )
    ...
    log.write_log(
        "Number of components > {}: {}. Last is {}".format(
            zero, len(selection_not_zero), selection_not_zero[-1]
        )
    )
    ...
    with PdfPages(output + "_all.pdf") as pdf:
        plt.fill_between(
            freqs,
            mean_periodogram[0] - std_periodogram[0],
            mean_periodogram[0] + std_periodogram[0],
        )
        ...
        class Psd:
            ...
            def ffpsd(self, w, single=False):
                WF = int(round(w / 1000.0 * self.DT_FS * len(self.freqs_THz) * 2.0))
                ...
                try:
                    for i in range(self.ucospectrum.shape[0]):
                        for j in range(self.ucospectrum.shape[1]):
                            if not single:
                                ffc = st.md.tools.filter.runavefilter(
                                    self.mcospectrum[i, j], WF
                                )
                            else:
                                ffc = st.md.tools.filter.runavefilter(
                                    self.ucospectrum[i, j], WF
                                )
        ...
        for i in range(periodograms.shape[0]):
            ...
            all_normalized[
                i * len(selection_not_zero) : (i + 1) * len(selection_not_zero)
            ] = (
                periodograms[i, 0, selection_not_zero]
        ...
        log.write_log(
            "Statistical test results (psd(0), psd(1), psd(all but 0)): {}".format(
                [ks__0, ks__1, ks_all]
            )
        )
        ...
        plt.fill_between(
            np.arange(mean_cepstral.shape[1]),
        ...
        plt.title(
            "Convergence of cepstral result with theoretical and statistical errors"
        )
...
def plt_hist_single_psd(data1, dof, nbins=None):
    ...
    plt.text(0.5, 0.6, text, transform=fig.transFigure)

    return ks_0


def plt_psd_with_zoom(
    jf,
    j2=None,
    j2pl=None,
    f_THz_max=None,
...
def plt_psd(
                ...
                k_SI_max2 = (
                    np.max(
                        np.real(
                            jf.cospectrum[0, 0][
                                : int(
                                    jf.freqs_THz.shape[0] * f_THz_max / jf.freqs_THz[-1]
                                )
                            ]
                        )
                        * jf.kappa_scale
    ...
    axes.plot(jf.freqs_THz, jf.psd * jf.kappa_scale * 0.5, lw=0.2, c="0.8", zorder=0)
    axes.plot(jf.freqs_THz, jf.fpsd * jf.kappa_scale * 0.5, c=c[0], zorder=2)
    if j2 != None:
        axes.axvline(x=j2.Nyquist_f_THz, ls="--", c="k", dashes=(1.4, 0.6), zorder=3)
    if j2pl != None:
        axes.plot(j2pl.freqs_THz, j2pl.cepf.psd * j2pl.kappa_scale * 0.5, c=c[2])
    try:
        axes.plot(
            jf.freqs_THz,
            np.real(jf.cospectrum[0, 0]) * jf.kappa_scale * 0.5,
            c=c[3],
            lw=1.0,
            zorder=1,
        )
    ...
    if nyq != None and nyq < f_THz_max:
        axes.annotate(
            ...
            xy=(
                nyq,
                (k_SI_max - jf.fpsd[idxnyq] * jf.kappa_scale * 0.5) / 7
                + jf.fpsd[idxnyq] * jf.kappa_scale * 0.5,
            ),
            xytext=(
                nyq,
                (k_SI_max - jf.fpsd[idxnyq] * jf.kappa_scale * 0.5) / 7
