...
class MaxLikeFilter:
    ...
    ** INTERNAL VARIABLES:
    ...
    def maxlike(self, data=None, model=None, n_parameters=None, likelihood=None, solver=None, mask=None,
        ...
        res = minimize(fun=self.log_like, x0=guess_par, args=(self.model, omega, self.omega_fixed, self.data, nu, ell),
                       method=self.solver, **minimize_kwargs,
                      )
    ...
    def guess_data(self, data, omega, omega_fixed, ell, nu, window=10, loglike='wishart'):
        ...
        try:
            ...
            log.write_log((f'Guessing data with a running average with a {window} large window'
                           f' failed with exception: {e}. Window changed to 10.'))
            window = 10
            guess_data = np.array([runavefilter(c, window) for c in data.reshape(-1, shape[-1])]).reshape(shape)
    ...
    def log_likelihood_wishart(self, w, model, omega, omega_fixed, data_, nu, ell, eps=1e-3):
        ...
        log_pdf = (0.5 * ((n - p - 1) * np.log(detX) - trinvV_X - n * np.log(detV) - n * p * np.log(2)) - logG)
