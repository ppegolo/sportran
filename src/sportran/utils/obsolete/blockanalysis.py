...
class MDBlocks(object):
    def __init__(
        self, traj, DT_FS, TSKIP, FILTER_WIDTH_T, temperature, volume, units, GUI=False
    ):
        ...
        self.units = units
        if self.units == "real":
            self.tau_scale = scale_kappa_REALtoSI(
                self.Temperature, self.Volume, self.DT_FS
            )
        elif self.units == "metal":
            self.tau_scale = scale_kappa_METALtoSI(
                self.Temperature, self.Volume, self.DT_FS
            )
        else:
        ...
        self.y_big = st.md.tools.filter_and_sample(
            traj,
            self.FILTER_WIDTH_T,
            self.TSKIP,
            "rectangular",
        ...
        log.write_log(
            " NYQUIST_F    = {:10g} = {:10g} THz".format(
                self.NYQUIST_F, self.NYQUIST_F / DT_FS * 1000
            )
        )
    ...
    def segment_trajectory(self, BLOCK_SIZE_T):
        ...
        self.BLOCK_SIZE = (
            BLOCK_SIZE_T / self.TSKIP
        )  # block width (in original time steps)
        ...
        self.block = [
            st.md.MDSample(
                traj=self.y_big[L * self.BLOCK_SIZE : (L + 1) * self.BLOCK_SIZE]
            )
            for L in range(self.N_BLOCKS)
        ]

    def cepstral_analysis(
        self, aic_type="aic", Kmin_corrfactor=1.0, bayes_p=False, density_grid=None
    ):
        ...
        if self.MULTI_COMPONENT:
            log.write_log(
                " N_EQUIV_COMPONENTS = {:10d}".format(self.N_EQUIV_COMPONENTS)
            )
            self.ck_THEORY_var, self.psd_THEORY_mean = (
                st.md.cepstral.multicomp_cepstral_parameters(
        ...
        for L in range(self.N_BLOCKS):
            if self.MULTI_COMPONENT:
                self.block[L].compute_psd(
                    DT=self.TSKIP, DT_FS=self.DT_FS, average_components=True
                )
                self.block[L].cepf = st.md.CepstralFilter(
                    self.block[L].logpsd,
                    ck_theory_var=self.ck_THEORY_var,
                    psd_theory_mean=self.psd_THEORY_mean,
                    aic_type=aic_type,
                    Kmin_corrfactor=Kmin_corrfactor,
                    normalization=self.BLOCK_SIZE,
                )
            else:
            ...
            if self.bayes_p:
                ...
                if density_grid is not None:
                    ...
                    self.block[L].cepf.compute_logtau_density(
                        method="ba", only_stats=False, density_grid=density_grid
                    )
                else:
                    self.block[L].cepf.compute_logtau_density(
                        method="ba", only_stats=True
                    )
            if self.GUI:
                progbar.value = float(L + 1) / self.N_BLOCKS * 100.0
    ...
    def cepstral_analysis_kappa(
        ...
        bayes_p=False,
        density_grid=None,
    ):  # need also "other", a class with the charge current!
        """Perform the Cepstral Analysis on all blocks."""
        ...
        if self.MULTI_COMPONENT:
            ...
            self.ck_THEORY_var, self.psd_THEORY_mean = (
                st.md.cepstral.multicomp_cepstral_parameters(
                    self.BLOCK_NFREQS, self.N_EQUIV_COMPONENTS - 1
                )
        ...
        for L in range(self.N_BLOCKS):
            if self.MULTI_COMPONENT:
                ...
                self.block[L].cepf = st.md.CepstralFilter(
                    ...
                    aic_type=aic_type,
                    Kmin_corrfactor=Kmin_corrfactor,
                )  # , normalization=self.BLOCK_SIZE) #removed (personal comunication with Loris)
            else:
                self.block[L].compute_kappa(
                    other=other.block[L], DT=self.TSKIP, DT_FS=self.DT_FS
                )  # different method call!
                self.block[L].cepf = st.md.CepstralFilter(
                    self.block[L].logpsd,
                    aic_type=aic_type,
    ...
    def compute_averages(self):
        ...
        log.write_log(
            "   max[AIC_Kmin]      =  {:12d}".format(np.max(list(self.aic_Kmin())))
        )
        log.write_log(
            "   AIC_Kmin           =  {:12.3f} +/- {:8f}".format(
                self.aic_Kmin_ave, self.aic_Kmin_std
            )
        )
        ...
        if self.bayes_p:
            ...
            self.avep_aic_KAVE, self.avep_aic_KSTD = st.md.aic.grid_statistics(
                np.arange(self.BLOCK_NFREQS), self.avep_aic
            )
            log.write_log(
                "   AIC_weight_distr   =  {:12.3f} +/- {:8f}".format(
                    self.p_aic_KAVE, self.p_aic_KSTD
                )
            )
            log.write_log(
                "       ave_AIC_w check:  {:12.3f} +/- {:8f}".format(
                    self.avep_aic_KAVE, self.avep_aic_KSTD
                )
            )
        ...
        log.write_log(
            "\n   flogtau[@AIC_Kmin] =  {:12f} +/- {:8f}  (SW p-value = {:5f})".format(
                self.flogtau_Kmin_ave,
                self.flogtau_Kmin_std,
                self.flogtau_Kmin_SW_pvalue,
            )
        )

        self.ftau_Kmin_ave = np.nanmean(list(self.ftau_Kmin()))
        self.ftau_Kmin_std = (
            self.flogtau_Kmin_std * self.ftau_Kmin_ave
        )  # np.std(list(self.ftau_Kmin()), ddof=1)
        log.write_log(
            "   ftau[@AIC_Kmin]    =  {:12f} +/- {:8f}".format(
                self.ftau_Kmin_ave, self.ftau_Kmin_std
            )
        )
        ...
        self.flogtau_THEORY_std_aveKmin = self.logtau_THEORY_std[
            int(round(self.aic_Kmin_ave))
        ]
        self.ftau_THEORY_std_aveKmin = (
            self.flogtau_THEORY_std_aveKmin * self.ftau_Kmin_ave
        )
        self.FTAU_THEORY_std_aveKmin = (
            self.ftau_THEORY_std_aveKmin * 0.5 * self.tau_scale
        )
        log.write_log(
            "\n   THEORY_STD_flogtau[@ave_AIC_Kmin] =    {:8f}  (errcheck: {:8f} +/- {:8f})".format(
                self.flogtau_THEORY_std_aveKmin,
                np.nanmean(list(self.flogtau_Kmin_THEORYstd())),
                np.nanstd(list(self.flogtau_Kmin_THEORYstd())),
            )
        )
        log.write_log(
            "   THEORY_STD_ftau[@ave_AIC_Kmin]    =    {:8f}".format(
                self.ftau_THEORY_std_aveKmin
            )
        )
        log.write_log(
        ...
        if self.bayes_p:
            ...
            self.flogtau_avedensity_XAVE, self.flogtau_avedensity_XSTD = (
                st.md.aic.grid_statistics(self.density_grid, self.flogtau_avedensity)
            )
            log.write_log(
                "\n   flogtau[AIC_w]     =  {:12f} +/- {:8f} (errcheck: {:8f} +/- {:8f})".format(
                    self.flogtau_density_XAVE,
                    self.flogtau_density_XSTD,
                    np.mean(np.mean(list(self.flogtau_density_xstd()))),
                    np.std(list(self.flogtau_density_xstd())),
                )
            )
            log.write_log(
                "   flogtau[ave AIC_w] =  {:12f} +/- {:8f}".format(
                    self.flogtau_avedensity_XAVE, self.flogtau_avedensity_XSTD
                )
            )
            ...
            log.write_log(
                "   ftau[AIC_w]        =  {:12f} +/- {:8f}".format(
                    self.ftau_density_XAVE, self.ftau_density_XSTD
                )
            )
    ...
    def slogpsd_histogram(self, XBINW=20, NYBINS=100, YEDGE1=-8.0, YEDGE2=8.0):
        ...
        for f in range(self.BLOCK_NFREQS / XBINW):
            self.slogpsd_h[f, :], self.slogpsd_h_yedges = np.histogram(
                np.transpose(
                    list(self.slogpsd())[f * XBINW : (f + 1) * XBINW][:]
                ).flatten(),
    ...
    def spsd_histogram(self, XBINW=20, NYBINS=200, YEDGE1=0.0, YEDGE2=10.0):
        ...
        for f in range(self.BLOCK_NFREQS / XBINW):
            self.spsd_h[f, :], self.spsd_h_yedges = np.histogram(
                np.transpose(
                    list(self.spsd())[f * XBINW : (f + 1) * XBINW][:]
                ).flatten(),
