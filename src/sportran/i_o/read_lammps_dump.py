...
def get_natoms(filename: str) -> int:
    f = open(filename, "r")
    line = f.readline()
    while line:
...
class LAMMPS_Dump(object):
    ...
    Example
    -------
    ``traj = LAMMPS_Dump(filename, preload=False)`` ``traj.read_timesteps(10,
    start_step=0, select_ckeys=['id,xu,yu,vu'])`` ``traj.read_timesteps((10, 30, 2))``
    ``print(traj.data)``
    """
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.all_timesteps: list[int] = []
        self._open_file()
        self._read_ckeys(group_vectors, preload_timesteps)
        # self.MAX_NSTEPS = data_length(self.filename) log.write_log("Data length = ",
        # self.MAX_NSTEPS)
    ...
    def _read_ckeys(
        self, group_vectors: bool = True, preload_timesteps: bool = True
    ) -> None:
                    ...
                    for i in range(2, len(values)):
                        if group_vectors:
                            bracket = is_vector_variable(
                                values[i]
                            )  # get position of left square bracket
                        else:
                            bracket = 0
                        if bracket == 0:  # the variable is a scalar
                            ...
                            if key[:2] == "c_":  # remove 'c_' if present
                                key = key[2:]
                            self.all_ckeys[key] = np.array([i - 2])  # -2 offset
        ...
        if self.preload_timesteps:
            ...
            log.write_log(
                " ** No timesteps pre-loaded. Be careful in the selection. **"
            )
    ...
    def _set_ckey(self, select_ckeys: list[str] | None = None) -> None:
        """
        Set the ckeys to read from the selected, checking the available ones. If
        select_ckeys is not passed, then use the already selected ones, or all the
        available ones if no selection was previously made.
        """
    ...
    def _set_timesteps(self, selection: Any, start_step: int = -1) -> None:
        ...
        INPUT:  N              -->  Read the next N steps (DELTA_TIMESTEP is assumed)
                ...
                (10,30)        -->  Read from TIMESTEP 10 to 30 (10,30,2)      -->  Read
                every 2 steps from TIMESTEP 10 to 30"""
        if start_step == -1:
        ...
        if np.isscalar(selection) or (
            len(selection) == 1
        ):  # select N steps from start one
            first = start_step
        ...
        if step is None:
            ...
            log.write_log(
                "Warning: step is not a multiple of the detected DELTA_TIMESTEP. You may get errors."
            )
        if first % step != 0:
        ...
        if self.preload_timesteps:
            for step in self.select_timesteps:
                if step in self.all_timesteps:
                    self.timestep.append(
                        step
                    )  # make list of available selected-timesteps
                else:
                    log.write_log("Warning: timestep # {:d} not found.".format(step))
        else:
            self.timestep = (
                self.select_timesteps.tolist()
            )  # use all the selected (be careful)
        ...
        for istep in range(self.nsteps):
            for key, idx in self.ckey.items():
                if key == "element":  # this should be improved
                    self.data[istep][key] = np.zeros(
                        (self.NATOMS, len(idx)), dtype="S8"
                    )
                else:
                    self.data[istep][key] = np.zeros(
                        (self.NATOMS, len(idx)), dtype="float64"
                    )
        return

    def _gototimestep(self, start_step: int, fast_check: bool = True) -> None:
        """
        ...
        if start_step >= 0:
            if start_step <= self.current_timestep:
                # or (self.current_timestep == -1):  # if start_step is before/equal the
                # current step
            ...
            # search until start_step is found     ***** MAY BE IMPROVED KNOWING THE N
            # OF LINES TO SKIP ******
            while True:
                line = self.file.readline()
                if len(line) == 0:  # EOF
                    raise EOFError(
                        "Warning (gototimestep):  reached EOF. Timestep {} NOT FOUND.".format(
                            goto_step
                        )
                    )
                if line == "ITEM: TIMESTEP\n":
                    ...
                    if self.current_timestep == goto_step:
                        while (
                            self.file.readline().find("ITEM: ATOMS") < 0
                        ):  # jump to the data part
                            pass
                        break
                    if (fast_check) and (self.current_timestep > goto_step):
                        raise Warning(
                            "Warning (gototimestep):  Timestep {} NOT FOUND up to current_step = {}. (To force check the whole trajectory set fast_check=False)".format(
                                goto_step, self.current_timestep
                            )
                        )
        else:
            pass
    ...
    def gototimestep(self, start_step: int, fast_check: bool = True) -> None:
        """
        Go to the start_step-th line in the time series (assumes step=1).
          start_step = -1  -->  ignore, continue from current step
                        0  -->  go to FIRST timestep N  -->  go to N-th timestep
    ...
    def read_timesteps(
        ...
        ``read_timesteps(10, select_ckeys=['id,xu,yu,vu'])`` ``read_timesteps((10,
        30))`` ``read_timesteps((10, 30, 2))``
        """
        if self._GUI:
        ...
        )  # set the timesteps to read  --> timestep
        ...
        for istep, step in enumerate(self.timestep):
            ...
            for nat in range(self.NATOMS):  # read data (may be unsorted)
                ...
                for key, idx in self.ckey.items():  # save the selected columns
                    atomid = (
                        int(values[atomid_col]) - 1
                    )  # current atom index (in LAMMPS it starts from 1)
                    if key == "element":  # this should be improved
                        self.data[istep][key][atomid, :] = np.array(
                            list(map(str, values[idx]))
                        )
                    else:
                        self.data[istep][key][atomid, :] = np.array(
            ...
            if (istep + 1) % progbar_step == 0:
                if self._GUI:
                    progbar.value = float(istep + 1) / self.nsteps * 100.0
                    progbar.description = "%g %%" % progbar.value
                else:
                    log.write_log(
                        "    step = {:9d} - {:6.2f}% completed".format(
                            istep + 1, float(istep + 1) / self.nsteps * 100.0
                        )
                    )
        if self._GUI:
            progbar.close()
