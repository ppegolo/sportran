...
def plt_resample(current, xf, axes=None, freq_units='THz', PSD_FILTER_W=None, FIGSIZE=None, mode='log'):
    """
    :param current:         current object to plot
    ...
    if not axes:
        figure, axes = plt.subplots(2, sharex=True, figsize=FIGSIZE)
        #print('axes ->', figure)
        axes = plot_periodogram(current=current, PSD_FILTER_W=PSD_FILTER_W, freq_units=freq_units, axes=axes,
                                     mode=mode, kappa_units=True)   # this also updates current.PSD_FILTER_W
    #print('->', axes)
